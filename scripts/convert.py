...
src_path = os.path.join(here, "../alkahest-obsidian")
dest_path = os.path.join(here, "../alkahest-mdbook/src")


files = [
    f[len(src_path) + 1 :]
    for f in glob.glob(f"{src_path}/**", recursive=True)
...
def convert_obsidian_links(source, files):
    ...
    def replace_link(match):
        ...
        return (
            f"[{link}]({get_relative_path(source, mf).replace(' ', '_')})"
            if mf
            else match.group(0)
        )
...
for f in files:
    if f.endswith(".md"):
        ...
        shutil.copy2(
            f"{src_path}/{f}", f"{dest_dir}/{os.path.basename(f).replace(' ', '_')}"
        )
